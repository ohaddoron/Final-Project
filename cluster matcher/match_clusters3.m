function [match_score,result] = match_clusters3( fpath,clusters2remove,...
...
%% read spikes
spk1 = read_spikes( fname1 ,nchans,template_time_length,clu1,res1,normlize_spiks);
[nchans,nSamples,nSpikes1] = size(spk1);
spk2 = read_spikes( fname2 ,nchans,template_time_length,clu2,res2,normlize_spiks);
[~,~,nSpikes2] = size(spk2);
...
mask1=zeros(nchans,nSamples,nClusters1);
N1=zeros(nClusters1,1);
ss1=zeros(nClusters1,1);
mmu1=zeros(nClusters1,1);
for iClusters1 = 1 : nClusters1
    ...
    tmp=mu1(:,:,iClusters1);
    ss1(iClusters1)=std(tmp(:));
    mmu1(iClusters1)=mean(tmp(:));
    mask1(:,:,iClusters1)=abs(mu1(:,:,iClusters1)-mmu1(iClusters1))>2*ss1(iClusters1);
end
idx2=false(nSpikes2,nClusters2);
mu2=zeros(nchans,nSamples,nClusters2);
sd2=zeros(nchans,nSamples,nClusters2);
mask2=zeros(nchans,nSamples,nClusters2);
N2=zeros(nClusters2,1);
ss2=zeros(nClusters2,1);
mmu2=zeros(nClusters2,1);
for iClusters2 = 1 : nClusters2
    ...
    tmp=mu2(:,:,iClusters2);
    ss2(iClusters2)=std(tmp(:));
    mmu2(iClusters2)=mean(tmp(:));
    mask2(:,:,iClusters2)=abs(mu2(:,:,iClusters2)-mmu2(iClusters2))>2*ss2(iClusters2);
end
...
%             spk2(idx2(:,iClusters2),:,:),'Vartype','unequal');
        tmp_score=((mu1(:,:,iClusters1)-mmu1(iClusters1))./ss1(iClusters1))...
            .*((mu2(:,:,iClusters2)-mmu1(iClusters1))./ss2(iClusters2));
        match_score(iClusters1,iClusters2)=mean(mean(tmp_score));
%         figure;
%         subplot(2,2,3)
